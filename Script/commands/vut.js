...
module.exports.config = {
  ...
  hasPermission: 0,
  credits: "Sadhin ⚡",
  description: "When a mentioned user says something, a ghost message will appear, with admin mode support.",
  commandCategory: "fun",
...
const adminModeFile = __dirname + "/vut_admin_mode.json"; // Stores admin mode status per thread
...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  const senderID = event.senderID; // The user who sent the message

  let status = fs.existsSync(dataFile) ? JSON.parse(fs.readFileSync(dataFile)) : {};

  // Check if vut mode is active for this thread and if the senderID is one of the targeted UIDs
  if (status[threadID] && status[threadID].includes(senderID)) {
...
module.exports.run = async function ({ api, event, args }) {
  ...
  // UID check for command execution
  ...
  let status = fs.existsSync(dataFile) ? JSON.parse(fs.readFileSync(dataFile)) : {};
  if (!status[threadID]) status[threadID] = []; // Initialize array if threadID not present

  if (args[0] === "off") {
    delete status[threadID]; // Remove the thread's entry
